...
##### 7. 应用申请后，如果处于审核周期，在待审核状态时可否进行测试？
...
-  showInterstitial(false) 方法中的参数代表了是否自动展示，如果填true在插屏广告准备完成后就会自动展示。请参考文档 [展现插屏广告](http://doc.ssp.yumimobi.com/AndroidSdk/index)相关内容。
    ![show方法](imgs/015.png)
##### 13. Android studio 接入使用远程依赖，远程依赖包下载失败，编译器提示：Could not find any matches for com.yumimobi.ads:mediation:3.*.*.+ as no versions of com.yumimobi.ads:mediation are available,应该如何处理？
...
##### 18. SDK接入没有问题，为什么换了正式ID后无法请求到广告？
- 新的广告位ID有可能还没有配置广告，联系玉米相关人员提供相应广告位  ID（可以发送邮件 support@yumimobi.com）确认配置。

##### 19. 调式插屏和激励视频的时候点击FETCH按钮出现interstitial onPrepareFailed LayerErrorCode:ERROR_NO_FAIL  MSG_CODE为5的错误是什么？
- 广告无填充，确认网络状态并多次尝试；也可联系运营人员提供相应广告位  ID（可以发送邮件 support@yumimobi.com）配置更多广告
...
##### 2. 为什么调试模式下，页面提示'warning  NOT ad config/平台显示为灰色？
...
##### 4.dome获取地址为主版本号，怎么获取对应SDK子版本号dome？
